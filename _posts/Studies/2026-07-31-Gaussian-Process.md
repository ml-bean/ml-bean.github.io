...
대표적인 예로 시계열을 생각할 수 있다.

시간 ($t$)마다 하나의 확률변수 ($X_t$)가 대응되며,  
...
여기서 $k(x_i, x_j)$는 입력 $x_i$와 $x_j$ 사이의 유사도를 수치화하는 커널 함수(Kernel Function)라고 한다.  
...
따라서 $[\Sigma]_{i,j} = Cov(f(x_i), f(x_j)) = k(x_i, x_j)$이고,
$k(x_i, x_j)$를 정의하는것이 우리의 믿음(belief, prior)을 모델에 부여하는 것이다.  

대각 원소 $k(x_i,x_i)$는 입력 $x_i$에서의 함숫값 $f(x_i)$가 가지는 분산을 나타내고,  
( $k(x_i,x_i) = \operatorname{Cov}(f(x_i), f(x_i)) = \operatorname{Var}(f(x_i))$ )  
비대각 원소 $k(x_i,x_j)$는 서로 다른 두 입력에서의 함숫값이 얼마나 유사하게 변화하는지를 나타낸다.
...
---
이번에는 커널 함수 $k(x_i,x_j)$가 어떤 식으로 우리의 사전 지식을 모델에 부여하는지 알아보겠다.
...
> 따라서 Gaussian Kernel은 유클리드 거리상 가까운 입력에서는 함숫값도 비슷하게 나타나며,  
> 입력이 조금 변하면 함숫값 역시 부드럽게 변할 것이라는 가정을 GP에 부여한다.
